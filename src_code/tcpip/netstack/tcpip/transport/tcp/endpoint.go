...
// Readiness 返回端点的当前读数据的准备情况。例如，如果设置了waiter.EventIn，则端点可立即读取。
...
// new connections.
// Listen 表示进入监听状态，允许新的连接建立，backlog 表示最大的半连接缓存数目
...
// Accept 返回新的tcp连接。必须先监听才能接收新的tcp连接。
